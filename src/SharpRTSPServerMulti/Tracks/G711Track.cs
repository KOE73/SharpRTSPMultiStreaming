...
{
    public PCMUTrack(string uri) : base(uri)
    {
    }
    ...
    {
        sdp.AppendLineRfc($"m=audio 0 RTP/AVP {PayloadType}");
        AppendControlRfc(sdp);
        sdp.AppendLineRfc($"a=rtpmap:{PayloadType} {Codec}/{SamplingRate}");
        return sdp;
    }
...
{
    public PCMATrack(string uri) : base(uri)
    {
    }
    ...
    {
        sdp.AppendLineRfc($"m=audio 0 RTP/AVP {PayloadType}");
        sdp.AppendLineRfc($"a=control:trackID={ID}");
        sdp.AppendLineRfc($"a=rtpmap:{PayloadType} {Codec}/{SamplingRate}");
        return sdp;
    }
