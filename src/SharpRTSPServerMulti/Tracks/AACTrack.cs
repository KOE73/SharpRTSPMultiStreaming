...
public class AACTrack : TrackBase
{
    ...
    public AACTrack(string uri, int samplingRate, int channels) : base(uri)
    {
    ...
    /// <param name="samplingRate">Audio sampling rate.</param>
    /// <param name="channels">Number of audio channels.</param>
    public AACTrack(string uri, byte[] configDescriptor, int samplingRate, int channels) : this(uri, samplingRate, channels)
    {
    ...
    {
        sdp.AppendLineRfc($"m=audio 0 RTP/AVP {PayloadType}"); // <---- Payload Type 0 means G711 ULAW, 96+ means dynamic payload type
        AppendControlRfc(sdp);
        sdp.AppendLineRfc($"a=rtpmap:{PayloadType} {Codec}/{SamplingRate}/{Channels}");
        sdp.AppendLineRfc($"a=fmtp:{PayloadType} profile-level-id={GetAACProfileLevel(SamplingRate, Channels)}; " +
