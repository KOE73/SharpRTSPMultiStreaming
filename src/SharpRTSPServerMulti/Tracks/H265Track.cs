...
public class H265Track : TrackBase
{
    ...
    public H265Track(string uri, int clock = DEFAULT_CLOCK) : base(uri)
    {
    ...
    /// <param name="clock">H265 clock. Default value is 90000.</param>
    public H265Track(string uri, byte[] vps, byte[] sps, byte[] pps, int clock = DEFAULT_CLOCK) : this(uri, clock)
    {
    ...
    public override StringBuilder BuildSDP(StringBuilder sdp)
    {
        ...
        sdp.AppendLineRfc($"a=fmtp:{PayloadType} sprop-vps={vps_str}; sprop-sps={sps_str}; sprop-pps={pps_str}");
        return sdp;
    }
