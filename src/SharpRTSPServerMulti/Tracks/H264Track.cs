...
public class H264Track : TrackBase
{
    ...
    public H264Track(string uri, int profileIdc = 77, int profileIop = 0, int level = 42, int clock = DEFAULT_CLOCK) : base(uri)
    {
    ...
    /// <param name="level">H264 Level. Default value is 42.</param>
    /// <param name="clock">H264 clock. Default value is 90000.</param>
    public H264Track(string uri, byte[] sps, byte[] pps, int profileIdc = 77, int profileIop = 0, int level = 42, int clock = DEFAULT_CLOCK) : 
        this(uri, profileIdc, profileIop, level, clock)
    {
    ...
    public override StringBuilder BuildSDP(StringBuilder sdp)
    {
        ...
        sdp.AppendLineRfc($"m=video 0 RTP/AVP {PayloadType}");
        AppendControlRfc(sdp);
        sdp.AppendLineRfc($"a=rtpmap:{PayloadType} {Codec}/{VideoClock}");
        sdp.AppendLineRfc($"a=fmtp:{PayloadType} profile-level-id={profileLevelIdStr}; sprop-parameter-sets={spsStr},{ppsStr}");
        return sdp;
    }
        ...
        for(int x = 0; x < samples.Count; x++)
        {
            ...
            if(fragmenting)
            {
                ...
                while(dataRemaining > 0)
                {
                    ...
                    // For H264 we need https://www.rfc-editor.org/rfc/rfc6184#page-29
