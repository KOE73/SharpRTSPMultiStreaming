...
namespace SharpRTSPServerMulti
{
    ...
    public class RTSPServer : IRtpSender, IDisposable
    {
        ...
        /// SSRC.
        /// </summary>
        public uint SSRC { get; set; } = (uint)Utilities._rand.Next(0, int.MaxValue); // 8 hex digits
        ...
        // URI → список треков (видео/аудио/несколько видео)
        private readonly Dictionary<string, List<ITrack>> _tracks = new();

        public void AddTrack(ITrack track)
        {
            if(!_tracks.ContainsKey(track.Uri))
                _tracks[track.Uri] = new List<ITrack>();

            track.Sink = this;
            _tracks[track.Uri].Add(track);
        }

        //public void AddVideoTrack(string uri, ITrack track)
        //{
        //    if(track != null)
        //    {
        //        track.Sink = this;
        ...
        //public void AddAudioTrack(string uri, ITrack track)
        //{
        //    if(track != null)
        //    {
        //        track.Sink = this;
        ...
        {
            var path = setupMessage.GetBasePath();
            ...
            else if(transport.LowerTransport == RtspTransport.LowerTransportType.UDP && !transport.IsMulticast)
            {
                ...
                udpPair.ControlReceived += (localSender, localE) =>
                {
                    ...
                    // TODO: Currently SharpRTSP only provides a parser for RTCP Sender Reports (PT=200).
                    // Waiting for future support of other RTCP packet types (e.g. 201=Receiver Report, 202=SDES, 203=BYE).
                    // For now we only wrap the raw data in RtcpPacket.
                    // RtcpPacket rtcpPacket = new RtcpPacket(localE.Data.Data.Span);
            ...
            if(transportReply != null)
            {
                ...
                lock(_connectionList)
                {
                    ...
                    var setupConnections = _connectionList.Where(connection => connection.Listener.RemoteEndPoint.Equals(listener.RemoteEndPoint));
                    foreach(var setupConnection in setupConnections)
                    {
                        ...
                        if(path is null || !_tracks.TryGetValue(path, out var tracks)) continue; // error case - path unknown

                        var trackId = setupMessage.GetTrackId();
                        //tracks.FirstOrDefault(t => t.ID == trackId);
                        if(trackId is null || setupConnection.Streams.Length < trackId) continue; // error case - track unknown

                        setupConnection.BaseUri = path; // !?

                        RTPStream stream = setupConnection.Streams[trackId.Value];

                        //RTPStream? stream = trackId switch
                        //{
                        //    int id when id == VideoTrack?.ID => setupConnection.Video,
                        //    int id when id == AudioTrack?.ID => setupConnection.Audio,
                        //    _ => null
                        //};


                        //if(stream == null) continue; // error case - track unknown
        ...
        private void HandleDescribe(RtspListener listener, RtspRequest message)
        {
            ...
            if(path is null || !_tracks.TryGetValue(path, out var track) || track.Any(o => !o.IsReady))
            {
        ...
        private string GenerateSDP(string uri)
        {
            ...
            if(!_tracks.TryGetValue(uri, out var tracks) || tracks.Count == 0)
                return string.Empty;
            ...
            sdp.Append("v=0\n");
            sdp.Append("c=user 123 0 IN IP4 0.0.0.0\n");
        ...
        public void FeedInRawRTP(string uri, int streamType, uint rtpTimestamp, List<Memory<byte>> rtpPackets)
        {
            ...
            lock(_connectionList)
            {
                ...
                RTSPConnection[] connections = _connectionList.Where(c => c.BaseUri == uri && c.Play).ToArray();
                foreach(RTSPConnection connection in connections) // ToArray makes a temp copy of the list. This lets us delete items in the foreach eg when there is Write Error
                {
